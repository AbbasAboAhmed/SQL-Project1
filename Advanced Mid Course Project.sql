...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN utm_campaign = 'nonbrand' THEN od.order_id           ELSE NULL END) AS nonbrand_orders,
    COUNT(DISTINCT CASE WHEN utm_campaign = 'brand'    THEN ws.website_session_id ELSE NULL END) AS brand_sessions, 
    COUNT(DISTINCT CASE WHEN utm_campaign = 'brand'    THEN od.order_id           ELSE NULL END) AS brand_orders
FROM website_sessions ws
	LEFT JOIN orders od
		ON ws.website_session_id = od.website_session_id
WHERE ws.created_at < '2012-11-27'
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN device_type = 'desktop' THEN od.order_id           ELSE NULL END) AS desktop_orders,
    COUNT(DISTINCT CASE WHEN device_type = 'mobile'  THEN ws.website_session_id ELSE NULL END) AS mobile_sessions, 
    COUNT(DISTINCT CASE WHEN device_type = 'mobile'  THEN od.order_id           ELSE NULL END) AS mobile_orders
FROM website_sessions ws
	LEFT JOIN orders  od
		ON ws.website_session_id = od.website_session_id
WHERE ws.created_at < '2012-11-27'
...
FROM first_test_pageviews ft
	LEFT JOIN website_pageviews wp
		ON wp.website_pageview_id = ft.min_pageview_id
...
FROM nonbrand_test_sessions_w_landing_pages nt
LEFT JOIN orders od
